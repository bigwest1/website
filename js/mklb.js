...
function _mklbOpen(mklbItem) {
    ...
    // Check for `data-video-src`
    let videoSrc = mklbItem.dataset.videoSrc || mklbItem.parentElement.dataset.videoSrc;

    if (videoSrc) {
        lightboxContainer.appendChild(_mklbAddVideo({ dataset: { videoSrc } }));
    } else if ('gallery' in mklbItem.dataset) {
...
function _mklbAddImage(item) {
    ...
    // Update all dynamic URL elements (h5 and h6 links)
    let dynamicElements = document.querySelectorAll('.dynamic-url');
    dynamicElements.forEach((el) => {
        el.addEventListener('click', (event) => {
            event.preventDefault(); // Prevent default browser behavior
            if (url !== '#') {
                window.open(url, '_blank'); // Open the specified URL
            }
        });

        if (el.tagName === 'A') {
            el.href = url; // Set the href for anchor tags
        }
    });
...
// Ensure event listeners are applied after the DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    // Attach the data-url to h5 and h6 links immediately on page load
    let contents = document.querySelectorAll('.content');
    contents.forEach((contentElement) => {
        let url = contentElement.dataset.url || '#'; // Get the URL

        // Update the h5 and h6 links inside the current content element
        let h5Link = contentElement.querySelector('h5 > a.dynamic-url');
        if (h5Link) {
            h5Link.href = url; // Set the href attribute
        }

        let h6Link = contentElement.querySelector('h6.dynamic-url');
        if (h6Link) {
            h6Link.addEventListener('click', (event) => {
    ...
    // Ensure dynamic-url links open their associated URL
    let dynamicElements = document.querySelectorAll('.dynamic-url');
    dynamicElements.forEach((element) => {
        element.addEventListener('click', (event) => {
            event.preventDefault(); // Prevent default browser behavior

            // Find the closest parent with a data-url attribute
            let contentElement = element.closest('.content');
            if (contentElement) {
                let url = contentElement.dataset.url || '#'; // Get the URL
                if (url !== '#') {
...
function _mklbAddVideo(item) {
    ...
    // Use 'data-video-src' instead of 'videoSrc'
    let source = document.createElement('source');
    source.src = item.dataset.videoSrc || item.src;  // Update to use 'data-video-src'
